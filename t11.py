...
@Function ：使用LoG处理之后，区域分割的效果是否会更好？
"""
import os
...
image_path = "ResultImages_t10\Gaussian_laplacian.bmp"
ExposureTime = 70000 #float(image_path.split('/')[-1].split('.')[0])
isGamma = 1
isMorphology = 0
...
if isGamma > 0 :
    ...
    if DEBUG:
        plt.imshow(image, cmap='gray'),plt.title('origin image(Gamma:'+str(isGamma)+')'),plt.show()
        visualize_3d_array(image,title='origin image(Gamma:'+str(isGamma)+')')

# 局部阈值分割
adaptive_threshold = cv2.adaptiveThreshold(image, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 21, -1)
if DEBUG:
...
if isMorphology==0:
    ...
    if DEBUG:
        plt.imshow(morphology_image, cmap='gray'),plt.title('threshold-none'),plt.show()
...
# 过滤面积大于阈值的连通组件
min_area_threshold = 0 if ExposureTime<1000 else 5 # math.pow((322-2.2*diameter)/precision,2) * 0.03
