...
def SingleExperimentAnalysis(dirs, title='', isMorphology=0, isGamma=0, mode =0):
    ...
    for dir in dirs:
        ...
        for img in imgs:
            ...
            mean_true_area, sqr_mean_magnification = SingleImageAnalysis(img_path,isMorphology=isMorphology,isGamma =isGamma, mode =0)
        ...
        # -------------------保存结果------------------------
        df_dir = pd.DataFrame(list(zip(magnifications_1D, true_area,equivalentDiameter)), columns=[dir+'mag', dir+'area',dir+'dia'], index=imgs)
    ...
    with pd.ExcelWriter('计算结果.xlsx',mode='a',engine='openpyxl',if_sheet_exists='replace') as writer:
